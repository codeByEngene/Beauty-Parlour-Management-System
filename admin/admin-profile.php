...
if (!isset($_SESSION['bpmsaid']) || strlen($_SESSION['bpmsaid']) == 0) {
    ...
    if(isset($_POST['remove_img'])) {
        ...
        if(!empty($oldimage)) {
            ...
            if(file_exists($path)) { unlink($path); } // Delete file from server
        }
...
<head>
    ...
    <style>
        ...
        .main-content { padding: 100px; margin-left: 300px; min-height: 100vh; }
        .profile-card { background: #fff; padding: 40px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); max-width: 650px; margin: 0 auto; }
        
        /* AVATAR CONTAINER */
        .avatar-container { position: relative; width: 120px; height: 120px; margin: 0 auto 30px; }
        .profile-avatar { 
            width: 100%; height: 100%; border-radius: 50%; object-fit: cover;
            background: linear-gradient(135deg, #e94e02, #ff8a50);
        ...
        .cam-icon { background: #6467c2; color: white; padding: 10px; border-radius: 50%; cursor: pointer; font-size: 14px; border: 2px solid #fff; display: block; }
        .remove-img-btn { 
        ...
        <form method="post" enctype="multipart/form-data">
            <div class="avatar-container">
                ...
                <?php else: ?>
                    ...
                    <button type="button" class="remove-img-btn" onclick="if(confirm('Remove profile photo?')) document.getElementById('removePhotoForm').submit();">
